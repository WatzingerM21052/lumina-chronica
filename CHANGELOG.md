...
- Living Library Feedback System, Phase C performance pass (issue #352, in progress): book/project cover images now load in parallel (`Task.WhenAll`, capped concurrency) instead of one at a time; cover routes now send `Cache-Control` only for `PUBLIC`-visibility books/projects (`private, no-store` everywhere else — a privacy boundary, not just a performance detail).

Backend: 345/345 Vitest tests passing (40 new). Frontend: 301/301 bUnit tests passing (77 new).
