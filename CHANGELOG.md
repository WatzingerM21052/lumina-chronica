...
- Living Library Feedback System, Phase B foundation (issue #349/#351): a real animated `LoadingIndicator` (the ring was previously the unmodified, never-animated Blazor WASM boot spinner) with new `Mode`/`Type` parameters; `SkeletonBlock`/`SkeletonCatalogCard` replacing Discover/PublicProfile's hand-rolled skeleton markup; a reusable button-state model (`ButtonBusyState`/`AsyncButtonRunner`/`AsyncButtonContent`, wired into Profile's save button) plus real press feedback (`scale(0.98)` on `:active`) for every button; a Toast/Snackbar system (`ToastService`/`ToastHost`, didn't exist before, wired into BookDetail's delete flow); `EmptyState` gaining `Ornament`/`ActionHref` (absorbing the hand-rolled `.empty-state-literary` pattern); and a new `ErrorMessage` component for page-level failures with a retry action (wired into BookDetail's load failure). Full detail, including two real bugs found only via live browser testing (a Toast/navigation render race; a delete failure that used to blank out the entire book page instead of showing an inline error) and a friendlier `ApiClient` error message replacing raw .NET exception text, in `documentation/Roadmap.md`'s Phase B entry.
- Living Library Feedback System, Phase C performance pass (issue #352, complete): book/project cover images now load in parallel (`Task.WhenAll`, capped concurrency) instead of one at a time; cover routes now send `Cache-Control` only for `PUBLIC`-visibility books/projects (`private, no-store` everywhere else — a privacy boundary, not just a performance detail); Discover's covers are lazy-loaded via `IntersectionObserver` instead of fetched all upfront; both cover routes support `ETag`/`If-None-Match` conditional GET, so a cover replace is picked up without waiting out the cache window.
- Living Library Feedback System, Phase D animation-technology decision (issue #353, complete): decided on optimized static WebP illustrations + CSS motion over SVG/Lottie (no vector source exists for the delivered mascot art, so that decided it). `LoadingIndicator` gained an `IllustrationPath` that swaps its spinner ring for the Lumina Archivist illustration in `Page`/`Fullscreen` mode when `Type="Library"` — reduced-motion aware, and toned down via a `filter` on the Dark Library theme so the candlelit scene doesn't glow against its near-black background. No existing call site was changed; wiring real pages is left to a later phase. Full four-criteria evaluation in `documentation/Architecture.md`.
- Living Library Feedback System, Phase E/F mascot integration (issue #366, complete): all five core Lumina illustrations now have exactly one real, live-verified page — Archivist (`Library.razor`), Reader (`Reader.razor`), Scribe (`BookUpload.razor`, `Mode="Fullscreen"`), Cartographer (`LocationDetail.razor`), Thinker (`Statistics.razor`). The Cartographer mapping changed mid-flight from the originally-proposed `ProjectDetail.razor` Orte tab, which turned out to be structurally unobservable (its loading branch can never render for a real user — see `documentation/Roadmap.md`). Also fixes a real bug found while verifying Scribe: `LoadingIndicator`'s `Fullscreen` mode used a theme-relative color for its loading text, making it nearly invisible in Dark Library.
- Living Library Feedback System, Phase G rollout (issue #372, complete): the Archivist and Reader illustrations now also show on `Home.razor`, `BookDetail.razor`, `ShelfDetail.razor`, `OfflineLibrary.razor`, `Shelves.razor` (Archivist) and `Bible.razor`, `Components/PdfReader/PdfReader.razor` (Reader) — 7 more pages, chosen after auditing every remaining `LoadingIndicator` call site for the same dead-branch risk #366 found on `ProjectDetail.razor`. The rest of the app's ~10 remaining call sites deliberately keep the plain spinner: some are structurally unobservable (same batching issue), the rest have no matching delivered mascot or no book/map/upload/processing/library semantics at all (auth pages, `StatusWidget`, `Profile.razor`).

Backend: 353/353 Vitest tests passing (48 new). Frontend: 312/312 bUnit tests passing (88 new).
