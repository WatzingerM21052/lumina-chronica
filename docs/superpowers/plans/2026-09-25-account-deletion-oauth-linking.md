...
- **Known gap, called out explicitly in Task 5 Step 4:** `_hasRealPassword` is hardcoded `true` since `UserProfile` doesn't currently expose whether an account is OAuth-only; this only affects whether the password field is shown, and whether it's required before the delete button enables, for such accounts (the backend already handles the OAuth-only case correctly regardless — `deleteUser` skips the password check itself). Not a blocker, flagged as a followup rather than silently guessed at.
