...
### Phase 3, part 1 — Motion system (issue #341) — complete; a11y audit + Architecture.md follow separately

Issue #341 bundles motion, a cross-theme accessibility audit, and an `Architecture.md` write-up. Split into two PRs on the advisor's recommendation: this phase is materially bigger than Phase 1/2 (touches `MainLayout`, shared by every page, plus a sticky-header layout change) and splitting keeps each PR's blast radius reviewable. **#341 stays open until both land.**
...
**Backend deploy note**: `.github/workflows/backend-deploy.yml` has no `CLOUDFLARE_API_TOKEN` repo secret configured and has been failing on every `workflow_dispatch` run since before this session (confirmed via `gh run list` — failures from 2026-08-05 and 2026-08-07, not introduced here). Backend changes in this session were deployed via local `wrangler deploy` instead. Worth fixing (add the secret) so this stops being a manual step, but out of scope for this PR.
