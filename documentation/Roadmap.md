...
### Phase 3, part 1 — Motion system (issue #341) — complete

Issue #341 bundles motion, a cross-theme accessibility audit, and an `Architecture.md` write-up. Split into two PRs on the advisor's recommendation: this phase is materially bigger than Phase 1/2 (touches `MainLayout`, shared by every page, plus a sticky-header layout change) and splitting keeps each PR's blast radius reviewable. Part 2 (below) landed the same day.

- [x] New motion tokens in `tokens.css`: `--motion-micro` (130ms, press/hover), `--motion-standard` (200ms, everyday UI), `--motion-reveal` (320ms, content/page reveals), `--ease-standard` (a fast-start/soft-landing cubic-bezier, no overshoot). Every transition Phase 1/2 already had (CatalogCard hover, follow-button, Discover's search/author-card/sort transitions, the global focus ring's non-color properties, `a`/`button` hovers) now references these instead of its own hardcoded duration, so the whole app moves at one consistent pace.
- [x] New `wwwroot/js/motion.js` — `initReveal` (scroll-triggered fade+rise for `[data-reveal]` sections, `IntersectionObserver`-based), `initHeaderCompact` (toggles `.is-compact` on the header via a scroll-sentinel observer, not a scroll-event listener), `replayPageEnter` (reflow-trick CSS animation restart on navigation).
- [x] **Progressive-enhancement safety, not CSS-default-hidden**: `[data-reveal]` sections render fully visible by default; only `initReveal` itself adds the hidden state (`.reveal-init`), right before observing. A blocked/slow/failed module load leaves content in its normal state — no permanently-invisible page is possible.
- [x] **Caught via advisor review before shipping, not after**: that safety net still let already-in-viewport content flicker on load, because `initReveal` runs after `await import(...)` — a real network fetch, not a same-tick call. On a slow/cold cache, already-painted above-fold content would snap invisible then fade back in, reading as a bug. Fixed by skipping `.reveal-init` entirely for any section already inside the viewport (`getBoundingClientRect().top < innerHeight`) at call time — those are left alone; only genuinely below-fold sections (Profile's later chapters, mainly) get the real scroll-triggered reveal. Discover's three top-level groups almost always fit above the fold on a normal viewport, so their `--reveal-delay` load-stagger (inline `style` per section, 0/60/120ms) is now mostly inert in practice — judged a smaller loss than a visible flicker on every cold load.
- [x] **`.app-header` is now `position: sticky` — a deliberate, app-wide layout change**, not a side effect of adding compaction. Compaction (smaller logo mark, reduced padding) only means something if the header stays on screen during scroll; making it sticky is what the issue's brief actually asked for, applied everywhere rather than only on Profile (harmless on short pages — nothing to compact if there's nothing to scroll past). `z-index: 10`, checked against every other `z-index` in the app (`ConfirmDialog` 100, toast/error banner 1000, various dropdowns 20 relative to their own component) — no conflicts.
- [x] Page-enter fade (`.page-enter`, `MainLayout.razor`) replays on every client-side navigation, gated on the path actually changing (not just any `LocationChanged` event — a deep link resolving to the already-shown page shouldn't flicker) and sequenced through `OnAfterRenderAsync` so it only touches the DOM after the new page's markup has actually rendered.
- [x] Follow button gained a restrained radial press-fade (`::after`, opacity-only) — explicitly not a literal Material ripple, per the issue's own wording.
- [x] Every new transition, plus the retrofitted Phase 1/2 ones, respects `prefers-reduced-motion` — checked in `motion.js` before any class is ever added (so reduced-motion users never pay for the dynamic import either), and mirrored as a CSS `@media` fallback for the OS setting changing live without a reload (which `matchMedia()` read once in JS wouldn't otherwise catch).
- [x] **A second real bug, caught by live-verify against the deployed site immediately after shipping the above (same-day follow-up)**: `initReveal` was wired from `MainLayout.razor`'s own `OnAfterRenderAsync(firstRender)`, which fires once for MainLayout's own first render — for a page with async data (Discover, PublicProfile) that's while the page is still showing its loading skeleton, before the real chapters/grid exist in the DOM. A descendant's own later re-render (once its data arrives) doesn't re-trigger the parent layout's `OnAfterRenderAsync`, so scroll-reveal silently never wired up at all. Confirmed live: none of PublicProfile's `[data-reveal]` sections had any reveal classes, including ones well below the fold. Fixed by moving `initReveal` into each consuming page's own `OnAfterRenderAsync` instead (idempotent, cheap to call every render). Re-verified live after the fix: below-fold chapters correctly show `reveal-init` then `reveal-visible` on scroll; above-fold content still shows neither (left alone, per the first fix).

**Not done in this part, deliberately deferred to part 2**: the cross-theme accessibility audit (keyboard nav, ARIA, contrast — including a real, computed finding that the global focus ring's gold color fails 3:1 contrast against light-theme paper/card backgrounds), rating-widget ARIA, search/skeleton accessibility, and the `documentation/Architecture.md` write-up the issue's Definition of Done requires.

Live-verified against real production data (2026-08-08) via direct `fetch()`/`javascript_tool` calls (not full UI clicks, for speed) plus visual spot-checks: scroll-reveal on Profile's chapters (both before and after the second bug's fix), header compaction on scroll, sticky positioning across the whole app, reduced-motion fallback, dark-library theme. Note: this environment can throttle/freeze `requestAnimationFrame`/Canvas work on backgrounded tabs, so exact motion *feel* (timing, easing) still needs the user's own eyes in a real foreground browser — flagged per the issue's own Definition of Done, not skipped.

Backend: unchanged. Frontend: 274/274 bUnit tests passing (2 new — `data-reveal` markup-hook coverage on Discover/PublicProfile; the reveal/compaction/page-enter behavior itself needs a real browser, not bUnit, and was live-verified instead).

**Backend deploy note (resolved same day)**: `.github/workflows/backend-deploy.yml` had no `CLOUDFLARE_API_TOKEN` repo secret and had been failing on every `workflow_dispatch` run since before this session (confirmed via `gh run list` — failures from 2026-08-05 and 2026-08-07, not introduced here). Backend changes in this session went out via local `wrangler deploy` while the gap existed. **The user added the secret the same day** (2026-08-08); a triggered `workflow_dispatch` run afterward succeeded end-to-end for the first time — the manual `wrangler deploy` workaround is no longer needed going forward.

### Phase 3, part 2 — Accessibility audit + `documentation/Architecture.md` (issue #341) — complete, closes #341

- [x] **Contrast fix**: new per-theme `--color-accent-text` token (`themes/*.css`). `--color-gold-accent` itself measures ~2.2–2.6:1 against light-theme (`classic-library`, `modern-light`) paper/card backgrounds — under WCAG's 3:1 floor for a UI component/focus indicator. `classic-library`/`dark-library` alias their existing `--color-primary`/`--color-gold-accent` (already ≥3:1 against their own backgrounds); `modern-light` — whose `--color-primary` is blue, so it can't reuse that token the way the other two do — gets a genuinely new darkened-gold hex (`#8c6a24`, ~4.6–5:1). Rerouted: the global focus-visible outline, `BookDetail.razor`'s filled rating stars, `CatalogCard`'s rating-star glyph. **Deliberately left untouched**: `--color-gold-accent` itself, and its purely decorative uses (heading double-rules, the `❦` empty-state ornament) — WCAG's contrast floor doesn't apply to decoration, and repainting it there would be an unrelated visual change.
- [x] **Keyboard nav**: `CatalogCard`'s owner-name `<span>` (`von {username}` on a Discover/Profile card) gained `tabindex="0" role="link"` + an Enter-key handler — it previously had only an `@onclick`, meaning it was invisible to Tab and had no way to be activated without a mouse at all. A real gap, not a theoretical one; caught during this pass's own review of every interactive element added across Phases 1–3, not by an automated tool.
- [x] **Rating-widget ARIA** (`BookDetail.razor`): star buttons gained `aria-pressed` (bound to an explicit `"true"`/`"false"` string, not a raw `bool` — Blazor's compiler picks the native-boolean-attribute rendering overload for a `bool`-typed value, which renders presence/absence, not the literal string a tri-state ARIA attribute needs; caught immediately by a bUnit test written alongside the change) and a toggle-aware `aria-label` ("Eigene Bewertung von N Sternen entfernen" only when that star *is* your current rating, since clicking it again removes rather than re-sets it — a generic "mit N Sternen bewerten" label would misdescribe the actual action in that case). The average-rating display (both here and in `CatalogCard`) gained a full-sentence `aria-label` in place of a bare `★ 4.5 · 2` glyph string.
- [x] **Search accessibility** (`Discover.razor`): new `_isSearchingUsers` flag drives `aria-live="polite"`/`aria-busy` plus a "Suche läuft…" message on the results region — set the instant typing happens (`OnUserSearchInput`), not just once the 400ms debounce timer fires, so the entire "nothing's visibly happened yet" window is covered, not just the network request's own duration.
- [x] **Skeleton accessibility**: both loading skeletons (Discover's book grid, PublicProfile's hero) stay `aria-hidden` — they're not real content and must never be read as such — but gained a paired `aria-busy="true"` and a screen-reader-only "wird geladen…" announcement via a new global `.visually-hidden` utility class, so a screen reader user isn't left in total silence during the load the way `aria-hidden` alone would leave them.
- [x] **`documentation/Architecture.md`** — untouched by this entire rework since Phase 1, despite issue #315's Definition of Done requiring it — gained three new decision-log entries (the same `Area | Decision | Reasoning` table format the rest of the document uses): `CatalogCard`/`Icon` architecture, the motion system (covering both bugs from part 1), and this accessibility pass. Also fixed the document's now-stale CI/CD row to reflect the `CLOUDFLARE_API_TOKEN` secret having been added. **Scope note**: this covers issue #315/#341's own decisions specifically, not a full backfill of every v3.0–v3.3 Community feature's architecture into this document — that's a separate, larger documentation debt not part of this issue.

Live-verified against real production data (2026-08-08): tab-order and Enter-key activation on `CatalogCard`'s owner link; rating-star `aria-pressed`/`aria-label` values via the browser's own accessibility tree, not just bUnit; search loading state appears immediately on typing. Contrast values were computed (WCAG relative-luminance formula) against each theme's actual token hex values, not eyeballed.

Backend: unchanged. Frontend: 277/277 bUnit tests passing (3 new this part: `BookDetail_RateStars_ExposeAriaPressedAndToggleAwareLabels`, `Discover_TypingAUsername_ShowsSearchingState`, `CatalogCard_OwnerLink_IsKeyboardOperable` — plus one existing skeleton test extended with `aria-busy` coverage).

**v3.4 Phase 3 complete. Issue #341 closed — all three phases of #315 shipped.**

## Living Library Feedback System — Phase B (issue #349/#351, complete, 2026-08-08/09)

Epic #349 started from a large user-authored vision ("Living Library Feedback System" — a mascot-driven loading/feedback overhaul) that was deliberately scoped down before any code was written: Phase A (audit, done inline), Phase B (foundation, this section), Phase C (performance — cover parallelization + PUBLIC/PRIVATE-aware caching, issue #352, shipped same day), Phase D (animation-technology decision, issue #353, still open — blocked on the mascot asset pipeline, not part of this section). Phase B shipped as six small, independently-mergeable PRs rather than one large rework, each adding exactly one reusable primitive plus exactly one reference integration — retrofitting every existing call site across the app is explicitly out of scope here, deferred to a later rollout phase.

- [x] **Slice 1 — `LoadingIndicator` animation + `Mode`/`Type`**: the ring was the unmodified, never-animated Blazor WASM boot spinner (a shared, unscoped `.loading-progress` rule in `app.css`, also used by `index.html`'s pre-boot loader) — given real rotation via a new `--motion-spinner` (1.1s) token. Added `Mode` (inline/button/section/page/fullscreen) and `Type` (library/search/reading/writing/map/upload/save/ai/processing, exposed as `data-loading-type`, inert until mascot assets exist). Both default to the pre-existing look — **zero of the ~21 existing call sites needed editing.**
- [x] **Slice 2 — `SkeletonBlock`/`SkeletonCatalogCard`**: replace `Discover`/`PublicProfile`'s hand-rolled skeleton markup with reusable primitives matching `CatalogCard`'s real shape (no layout shift once real content arrives).
- [x] **Slice 3 — Button states**: real press feedback (`scale(0.98)` on `:active`) for every plain button, previously only the Follow button had this. `ButtonBusyState`/`AsyncButtonRunner`/`AsyncButtonContent` add a reusable Idle→Loading→Success→Idle cycle (180ms delay-before-show, 1s success hold) wired into `Profile.razor`'s save button.
- [x] **Slice 4 — Toast/Snackbar**: `ToastService`/`ToastHost`, didn't exist at all before (0 hits). Wired into `BookDetail`'s delete flow.
- [x] **Slice 5 — `EmptyState` unification**: gained `Ornament`/`ActionHref`, absorbing the hand-rolled `.empty-state-literary` pattern `Discover`/`PublicProfile` had each duplicated.
- [x] **Slice 6 — `ErrorMessage` + two real bugs fixed**: new component for page-level failures with a retry action, wired into `BookDetail`'s load failure. Found live: a failed delete used to collapse the *entire* book page to just an error message (wrong field reused for two different failure states); `ApiClient.GetAsync` showed .NET's raw, unformatted exception text on any HTTP error instead of a friendly message.

**Two real bugs found only by live browser testing, not bUnit** (full detail in `documentation/Architecture.md`'s decision-log entry for this phase): a Toast-vs-navigation render race (fixed with a direct `StateHasChanged()` call *and* `await Task.Yield();` in the caller — either alone was insufficient), and the `_loadErrorMessage`/`_deleteErrorMessage` field-reuse bug above. A CSS-isolation regression was also caught **proactively, before merging** this time (a page-scoped margin rule that would have silently stopped applying once its markup moved into `EmptyState` — moved to `app.css` first) — the same bug class PR #313 shipped live back in v3.0.

Live-verified in a real local dev environment for the first time this session (`wrangler dev` + `dotnet run`, both previously only deployed-and-checked, not run locally) — set up specifically because bUnit structurally cannot verify computed CSS/animations, button-width layout shift, or cross-render-request races. Local D1 state needed catching up via `wrangler d1 migrations apply --local` (had drifted since migration 0004).
...
- [x] **Scribe → `BookUpload.razor`**: new `IllustrationPath` map entry (`Type=Upload` → `lumina-scribe.webp`). Used `Mode="Fullscreen"` here instead of `Page` — this loading state replaces just the submit button inside a form, not a whole page's content, so `Page`'s 40vh content area would have looked oversized/out of place; a full-viewport scrim blocking the whole form during the upload request fits the "blocking operation" framing `Fullscreen` was already designed for. **Found and fixed a real, previously-latent bug while verifying**: `Fullscreen` mode's text used `color: var(--color-bg-light)`, a theme-relative token (the lighter of that *theme's own* two backgrounds) — in Dark Library, `--color-bg-light` is `#251c14`, nearly the same dark brown as the scrim itself, making "Wird hochgeladen…" nearly invisible. `Fullscreen` had no real call site before this slice, so the bug was latent since Phase B. Fixed by switching to `--color-text-on-dark`, an existing token already used by `ToastHost`/`NotificationBell` specifically for "always-light text on a dark surface regardless of theme." Live-verified in both themes with a real upload (throwaway `.txt`, fetch frozen so nothing was actually created server-side, confirmed via `GET /api/books` returning empty afterward).
- [x] **Cartographer → `LocationDetail.razor`** (not `ProjectDetail.razor`'s Orte tab as originally proposed): `ProjectDetail.razor` loads all seven of its tabs' data in one `Task.WhenAll` inside `OnInitializedAsync`, and the whole page — including the tab buttons themselves — is gated on `_project is not null`, which only becomes true once that same `WhenAll` resolves. Practically, `_locations is null` (the branch the illustration would replace) can never be observed by a real user: by the time the tab buttons exist to click, `_locations` is already populated too. Found this live while trying to verify it — selectively froze only the `/locations` fetch expecting to reach the tab and see the loading state, but the whole page just stayed on its own top-level "Projekt wird geladen…" forever, proving the two loads can't be observed as separate states. `LocationDetail.razor` — a real, independently-routed page with its own single fetch — is the genuinely reachable Cartographer moment instead: new `IllustrationPath` map entry (`Type=Map` → `lumina-cartographer.webp`), its "Ort wird geladen…" gained `Mode="Page"` `Type="Map"`. Live-verified against a real throwaway location in both themes.
