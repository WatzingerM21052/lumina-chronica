...
- **`books`** — `id`, `owner_id` (FK → `users.id`), `title`, `author`, `description`, `cover_url` (R2 key, nullable), `language`, `genre`, `visibility` (`PRIVATE`/`SHARED`/`PUBLIC`, defaults `PRIVATE`), `created_at`, `updated_at`. Indexed on `owner_id` and `genre`. Per Teil 4 §46.1. All three visibility values are enforced as of v3.1: `PRIVATE` — owner only; `SHARED` — full read access (detail, file, progress, bookmarks) for any logged-in user, mutations stay owner-only (issue #316); `PUBLIC` — cover+metadata teaser for anyone including anonymous visitors, the file itself still requires being logged in as the owner (issue #300).
