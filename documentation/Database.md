...
- **`users`** — `id`, `username`, `email`, `password_hash`, `avatar_url`, `role_id` (FK → `roles.id`), `created_at`, `last_login`, `updated_at`, `deleted_at`. Columns per Teil 4 §45.1, extended with `updated_at`/`deleted_at` per the Technical Standards timestamp rule. Also `deleted_username`/`deleted_email` (`0023_account_deletion.sql`, see that migration's own entry below for what they hold).
...
- **`oauth_states`** — `state` (PK), `provider`, `expires_at`. The CSRF `state` param for the authorize redirect, persisted here (not in-memory) since the Worker is stateless/horizontally scaled and `/callback` may land on a different instance than `/start`. Deleted on read (single-use). Also `linking_user_id` (`0024_oauth_linking.sql`, see that migration's own entry below).
...
- **`profile_activities`** — `id`, `user_id` (FK → `users.id`), `type` (`TEXT`, `CHECK` in `BOOK_PUBLIC`/`PROJECT_PUBLIC`/`RATING_GIVEN`), `target_type` (`TEXT`, `CHECK` in `BOOK`/`PROJECT`), `target_id` (no FK — polymorphic target, same shape as the spec's own `comments` table design in §50.3), `rating` (nullable, a snapshot of the rating value at creation time, `RATING_GIVEN` only), `created_at`. A log of a profile owner's own public actions, rendered on `/u/{username}`'s "Aktivitäten" section (folded into `GET /api/users/:username/public`'s response, not a separate endpoint). Deliberately not named `activities` — `reading_activity` (migration `0007` above) already exists and is a different concept (per-day reading counts, not a public event log). `target_id` having no FK means `deleteBook`/`deleteProject` need an explicit cleanup line each (they have one) — a dangling row here wouldn't throw an FK error the way a real FK violation would, so it's covered by a dedicated regression test instead of relying on D1 to catch a missing cleanup line. As of migration `0021` (issue #315 Phase 3), a `RATING_GIVEN` row's `rating` can also be `NULL` for a different reason than "target deleted": the rater had `ACTIVITY_RATING_STARS` off at rating time, so the row records that a rating happened without the specific value — see that migration's note.

### `0019_comments.sql` (v3.3 — Comments/Notifications/Activities, Phase 2, issue #325)

- **`comments`** — `id`, `user_id` (FK → `users.id`), `target_type` (`TEXT`, `CHECK` in `BOOK`/`PROJECT`), `target_id` (no FK, polymorphic, same shape as `profile_activities`), `content`, `created_at`. Source spec §50.3's bare field list, resolved to Books + Projects only (no new public page type). Access is deliberately asymmetric: a book uses the same `isBookAccessibleTo` rule as reading it (owner, PUBLIC-logged-in, or SHARED-listed — not ratings' PUBLIC-only rule); a project is owner-or-PUBLIC only (`isProjectCommentableBy`), since projects have no share-list equivalent to books' `book_shares`. `deleteBook`/`deleteProject` both gained a `comments` cleanup line, same recurring bug class as `profile_activities`/`book_shares`/`ratings`.
- Note: `ProjectDetail.razor` (the only frontend surface for project comments) is reachable only by a project's own owner — `getProject` is strictly owner-only, unlike books' `getBook` — so non-owner PUBLIC-project commenting is a real, tested backend capability with no frontend path to reach it yet. See `Roadmap.md`'s v3.3 Phase 2 section for the full explanation.

### `0020_notifications.sql` (v3.3 — Comments/Notifications/Activities, Phase 3, issue #326)

- **`notifications`** — `id`, `user_id` (FK → `users.id`, the recipient), `type` (`TEXT`, `CHECK` in `FOLLOW`/`COMMENT`/`RATING`/`SHARE`), `actor_user_id` (FK → `users.id`, who triggered it), `target_type`/`target_id` (no FK, polymorphic, same shape as `profile_activities`/`comments`), `read_at` (nullable), `created_at`. In-app only, no email/push. Inserted via a single self-contained SQL statement (`buildNotificationInsert` in `notificationService.ts`) that guards both the self-notification case and the muted-preference case in its own `WHERE`/`NOT EXISTS` clause, so it's always safe to include in a `db.batch()` alongside the primary write it's triggered by (rating, commenting) — for the two `INSERT OR IGNORE` triggers (follow, share) it's issued only when the primary insert's `meta.changes > 0`, so a repeat/idempotent action doesn't spam a duplicate notification.
- **`user_preferences`** — `user_id` (FK → `users.id`), `type` (`TEXT`, `CHECK`, see below), `enabled` (`INTEGER` bool, default `1`), composite PK `(user_id, type)`. Row-per-type. `ACTIVITY_RATING` is a 5th type (beyond the base 4 notification triggers) reusing this same mechanism to gate `profile_activities`' `RATING_GIVEN` entries rather than a notification — added mid-Phase-3 via a follow-up `AskUserQuestion`, since a rating is the first activity entry that reveals a specific action rather than restating already-public state. Checked at **insert time**, not read time, in every case — muting never retroactively hides already-generated history.
  - **Defaults are not uniform.** `FOLLOW`/`COMMENT`/`RATING`/`SHARE` default to *enabled* when no row exists (opt-out) — that logic lives in `notificationService.ts`'s `PREFERENCE_DEFAULTS` map now, not "absence of a row always means enabled" as earlier phases had it. `ACTIVITY_RATING` and `ACTIVITY_RATING_STARS` default to *disabled* (opt-in): both surface something personal (an opinion) on a page anyone can view.

### `0021_activity_rating_stars.sql` (issue #315 Phase 3, follow-up raised by the user during redesign review)

- Adds `'ACTIVITY_RATING_STARS'` to `user_preferences.type`'s `CHECK` constraint via the standard SQLite recreate-and-copy (no `ALTER ... CHECK`), and flips `ACTIVITY_RATING`'s effective default from enabled to disabled (in code, not schema — see above). Splits "does a rating show up in my activity log at all" (`ACTIVITY_RATING`) from "does the specific star count show up when it does" (`ACTIVITY_RATING_STARS`) — the star *value* is the part that can read as an opinion about someone else's work and unfairly color another visitor's impression, more so than the bare fact that a rating happened. `activityService.recordRatingActivity` checks both at insert time and stores `rating = NULL` when the second is off, rather than only deciding whether to insert a row at all.

### `0023_account_deletion.sql` (Roadmap "Profile functions", 2nd of 3)

- **`users.deleted_username`/`users.deleted_email`** — nullable `TEXT`, added to the existing `users` table. `username`/`email` stay `UNIQUE` table-wide, not scoped to `deleted_at` (rebuilding the table to change that constraint is documented as broken against real D1 by `0005_oauth.sql`'s own note — `FOREIGN KEY constraint` error on `DROP TABLE`, since `users` has several incoming FKs). So `deleteUser` (`userService.ts`) instead overwrites the live `username`/`email` with a generated placeholder (`deleted-user-{id}` / `deleted-{id}@deleted.invalid`, freeing them immediately with zero constraint risk) and moves the original values into these two columns, so a later registration with the same email can find and restore this exact row (`authService.ts`'s `registerUser`'s deleted-account gate / `restoreUser`). Restoring clears both back to `NULL`.

### `0024_oauth_linking.sql` (Roadmap "Profile functions", 3rd of 3)

- **`oauth_states.linking_user_id`** — nullable `INTEGER` (FK → `users.id`, `ON DELETE CASCADE`), added to the existing `oauth_states` table. An already-authenticated user can link an additional Google/GitHub identity from their Profile page; `linking_user_id` rides along on the same state row that already survives the redirect round-trip to the provider and back. `NULL` means an ordinary login flow (the only case before this migration); set means the callback should link the identity onto that user instead of logging someone in.
- `deleteUser` (`userService.ts`) also deletes the caller's `oauth_identities` rows on account deletion (closing an OAuth-sign-in-survives-deletion hole — a soft-deleted account with a linked provider identity could otherwise still sign in through it, making deletion nearly a no-op for a purely-OAuth account) and, as defense-in-depth, any live `oauth_exchange_codes`/`oauth_states` rows tied to them (`WHERE user_id = ?` / `WHERE linking_user_id = ?` respectively — `oauth_states` has no `user_id` column for a normal login-flow row). Known, accepted tradeoff: linked identities do NOT reattach on `POST /api/auth/restore` (unlike every other owned row) — a restored account must re-link providers from Profile's "Verknüpfte Konten" section, acceptable since `restoreUser` always sets a fresh password.
