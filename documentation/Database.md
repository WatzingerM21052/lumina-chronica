...
- **`user_preferences`** — `user_id` (FK → `users.id`), `type` (`TEXT`, `CHECK` in `FOLLOW`/`COMMENT`/`RATING`/`SHARE`/`ACTIVITY_RATING`), `enabled` (`INTEGER` bool, default `1`), composite PK `(user_id, type)`. Row-per-type, absence of a row means enabled. The user's own explicit requirement (per-type enable/disable in settings) beyond the base 4 trigger types. `ACTIVITY_RATING` is a 5th type reusing this same mechanism to gate `profile_activities`' `RATING_GIVEN` entries (Phase 1) rather than a notification — added mid-Phase-3 via a follow-up `AskUserQuestion`, since a rating is the first activity entry that reveals a specific action rather than restating already-public state. Checked at **insert time**, not read time, in both cases — muting never retroactively hides already-generated history.
