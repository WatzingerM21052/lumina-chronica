...
| GitHub Pages + Blazor WASM deploy correctness | `.nojekyll` in the published output (GitHub Pages runs Jekyll by default, which ignores `_`-prefixed folders like Blazor's `_framework/`); `<base href="/lumina-chronica/" />` in `index.html` (must match the repo name); `404.html` = a copy of `index.html` so client-side routes survive a hard refresh. | None of the source documents mention Blazor+Pages deployment mechanics at all; without these three, the deployed app 404s on load or on refresh. |
